...
#if defined(NDEBUG)
# error "Buildingcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcf71d1211589062c6d913437d06615a8b64761ae601b10575a1360b642604491");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Buildingcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Buildingcoin Signed Message:\n";
...
{
    // Buildingcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Buildingcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Buildingcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Buildingcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Buildingcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Buildingcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4f7bc5734fc421f931939e95e2705fe44b7fc197c76a23e155b70928b9c6fe52");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "07/19/2020 cumplo 50 y en este 2020 se recordara por la pandemia mundial";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049908e1a01ab62d4c518eba4929b2444e3d3f4e2b9c15b23812980496605d1c6d73fee3ab67632bab9f2b12c5184200aaee03c7dbf2127fe51c5e6e3f89d4c074") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1595179961;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086451071;

        if (fTestNet)
        {
            block.nTime    = 1595179918;
            block.nNonce   = 385536568;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xff274824ece25d818b803ebd3ba435a2d101f53ec27cc6b884e110e4b631478f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Buildingcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BuildingcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BuildingcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BuildingcoinMiner(CWallet *pwallet)
{
    printf("BuildingcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("buildingcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BuildingcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BuildingcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BuildingcoinMiner, pwallet));
}
